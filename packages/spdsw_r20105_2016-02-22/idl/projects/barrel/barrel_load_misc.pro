...
pro barrel_load_misc, probe=probe, datatype=datatype, trange=trange, $
...
; specify VDATATYPES and VLEVELS for MISC 
vlevels = ['l2','l1']        ; Zeroth element is our default (require only one level)
vdatatypes = ['misc']   ; Require only one datatype at this level
...
IF (n_newvars GT 0) THEN BEGIN
    ...
    FOR i=0, N_ELEMENTS(tplotnames)-1 DO BEGIN
        ...
        IF size(/type, dl_str) EQ 8 && ((dl_str.cdf.vatt.var_type EQ 'data') OR $ 
            ...
            ; MISC-class variables have name structure: 
            ;   brlPP_VARIABLE 
            ; 
            ...
            ;
            ; and VARIABLE designates the relevant MISC-class variable
            ;   MISC_FrameGroup
            ;   MISC_Quality
            ;   Version
            ;   MISC_GPS_PPS
            ;   Payload_ID
            ;   MISC_Time_Model_Intercept [L1 only?]
            ;   MISC_Time_Model_Slope [L1 only?]
            ...
            CASE STRMID(tplot_var,breakpt) OF
                'FrameGroup': BEGIN
                        ...
                        new_name = STRMID(tplot_var,0,breakpt)+'MISC_'+$
